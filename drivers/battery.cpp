...
//
static const float ADC_TO_VBAT =
        (5.0f / 1023.0f) * ((56.0f + 10.0f) / 10.0f);
...
Result battery_task()
{
    ...
    filtered_vbat = (FILTER_ALPHA * vbat) + ((1.0f - FILTER_ALPHA) * filtered_vbat);
...
{
    if (!initialized || out_voltage == NULL)
        return RES_PARAM;
